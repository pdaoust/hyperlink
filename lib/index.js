 ...
 * @param  {Function} [options.skipFragmentFilter] Filter function to mark failed tests as skipped, but only if the URL check fails, not if the fragment check fails
 ...
 * @param  {Function} [options.todoServerErrorsFilter] Filter function to mark 5xx or ETIMEDOUTs for known flaky servers as todo rather than failing. If the server responds with a 2xx, 3xx, or 4xx, the normal filters and checks will apply.
 ...
 * @param  {String} [options.redirects = "check"] What to do about inefficient redirects. Values are "check", "skip", and "todo".
 ...
 * @param  {Number} [options.timeout = 30000] HTTP timeout in milliseconds
...
async function hyperlink(
  {
    ...
    skipFragmentFilter = returnFalse,
    todoFilter = returnFalse,
    todoServerErrorsFilter = returnFalse,
    recursive = false,
    internalOnly = false,
    pretty = false,
      ...
      const socket = (isTls ? tls : net)
        ...
        .on('error', (error) => {
          ...
          if (error.code === 'ENOTFOUND') {
            actual = `DNS missing: ${host}`;
          }

          reportTest({
            ...connectionReport,
            ok: false,
            todo: todoServerErrorsFilter(connectionReport),
            actual,
          });
        ...
        (error, res) => {
          if (error) {
            ...
            reportTest({
              ...loadReport,
              ok: false,
              todo: todoServerErrorsFilter(loadReport),
              actual,
            });
          ...
          // Slow down with a linear backoff if we're getting pushback from the server.
          if (status === 429) {
            return setTimeout(() => httpStatus(asset, attempt + 1)(callback), 500 * attempt);
          }
          ...
          if (attempt === 2 && status === 502) {
            return setTimeout(() => httpStatus(asset, attempt + 1)(callback), 1000);
          }

          const urlRedirects = res.request._redirect.redirects;
          if (urlRedirects.length > 0) {
            const log = [{ redirectUri: url }, ...urlRedirects].map(
              (item, idx, arr) => {
                if (arr[idx + 1]) {
            ...
            if (!shouldSkip(redirectReport) && redirects != "skip") {
          ...
          reportTest({
            ...
            actual: `${status} ${url}`,
            ok: false,
            todo: status >= 500 && status < 600 && todoServerErrorsFilter(loadReport),
          });
      ...
      for (const relation of asset.externalRelations) {
        ...
        if (fragment) {
          if (fragment === '#') {
            ...
            if (!shouldSkip(fragmentReport) && !skipFragmentFilter(fragmentReport)) {
